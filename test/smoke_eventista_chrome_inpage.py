"""Smoke test: Chrome thật (engine=chrome) + proxy + in-page Turnstile solve.

Đi đúng flow: mở trang (qua proxy) → btn-signin → "Đăng ký ngay" →
register form → inject widget riêng → chờ auto-solve/click → token?
"""
import asyncio
import sys
...
async def main() -> int:
    ...
    try:
        ...
        print("── solve in-page (Chrome thật + proxy) ──")
