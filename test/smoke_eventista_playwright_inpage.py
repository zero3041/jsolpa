"""Smoke test: Playwright Chromium engine + in-page Turnstile solve.

Đi đúng flow: mở trang → btn-signin → login dialog → "Đăng ký ngay" →
...
async def main() -> int:
    print("── launch Playwright Chromium ──")
    page, _handle, close = await _launch_browser(
        "playwright", headless=False, proxy=None
    )
    try:
        ...
        print("── solve in-page (Playwright Chromium) ──")
